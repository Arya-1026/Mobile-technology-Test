...
class TeamService {
  ...
  static Future<void> sendInviteByEmail({
    ...
    final duplicate = await _invites
        ...
        .get();
    if (duplicate.docs.isNotEmpty) {
      throw Exception('Энэ email рүү invite аль хэдийн илгээгдсэн байна');
    }
  ...
  static Future<void> linkPendingEmailInvites(User user) async {
    ...
    for (final doc in snap.docs) {
      ...
      await respondInvite(TeamInvite.fromDoc(await doc.reference.get()), true);
    }
  }

  static Future<void> respondInvite(TeamInvite invite, bool accepted) async {
    final user = _auth.currentUser;
    if (user == null || user.uid != invite.toUid) {
    ...
    await _db.runTransaction((transaction) async {
      ...
      final member = TeamMember(
        uid: user.uid,
        name: userData['name'] ?? user.displayName ?? invite.toName,
  ...
  static Future<void> saveMatchResult({
    ...
    await _writeResultForTeam(match.teamAId, match, winnerTeamId, score, status);
    await _writeResultForTeam(match.teamBId, match, winnerTeamId, score, status);
  }
